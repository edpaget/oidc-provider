...
1. **List tasks**: `rdm task list --project oidc-provider` to see open and in-progress tasks.
2. **Show details**: if a task slug was provided in `$ARGUMENTS`, run `rdm task show <slug> --project oidc-provider`. Otherwise, present the task list and ask the user which task to work on.
3. **Mark in-progress**: `rdm task update <slug> --status in-progress --no-edit --project oidc-provider`
